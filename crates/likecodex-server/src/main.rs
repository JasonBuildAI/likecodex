...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        // ── PTY Terminal API ───────────────────────────────────────────
        .route("/api/terminal/create", post(pty_create))
        .route("/api/terminal/:id/close", post(pty_close))
        .route("/api/terminal/list", get(pty_list))
        .route("/api/terminal/:id/resize", post(pty_resize))
        .route("/api/terminal/:id/ws", get(pty_ws_handler))
        .layer(cors)
...
async fn pty_create(
    ...
    pty::PtyManager::create_session(&state.pty_manager, session_id.clone(), cwd)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn pty_resize(
    ...
    // Resize is acknowledged; platform-specific impl is a future enhancement.
...
async fn handle_pty_ws(mut ws: WebSocket, pty: PtyManager, session_id: String) {
    ...
    let stdin_tx = match pty.get_session(&session_id).await {
        Some(tx) => tx,
        None => {
            let _ = ws.send(Message::Text("ERROR: session not found".into())).await;
            return;
        }
    };
    ...
    // The session's output_rx was consumed during creation.
    // For full duplex, we'd need a broadcast channel or a different architecture.
    // This is a placeholder that acknowledges the WebSocket connection.
    let _ = ws_sender
...
/// POST /proxy/resume — resume a saved session (existing).
