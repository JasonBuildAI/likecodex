...
/**
 * Unified chat core hook merging useChatLogic and useIdeChat.
 * Provides runPrompt, cancelPrompt, and event subscription in one place.
 */
export function useChatCore() {
  ...
  const runPrompt = useCallback(async (prompt: string, skillName?: string) => {
    ...
    isStreaming, currentSessionId, setCurrentSessionId, agentMode,
  ...
  const subscribe = useCallback(() => {
    unsubRef.current = subscribeEvents({
      onError: (err) => addToast({ type: 'error', message: `Event error: ${err.message}` }),
    });
