...
import { ModeCapsule } from '@/components/InputArea/ModeCapsule';
...
export const ChatInputPanel: React.FC<ChatInputPanelProps> = ({
  onSubmit,
  onStop,
  inputHistory: externalHistory,
  placeholder,
  showModeCapsule = true,
}) => {
  ...
  const inputHistory = externalHistory || internalHistory.current;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim() || isStreaming) return;
    internalHistory.current = [input, ...internalHistory.current].slice(0, 50);
    onSubmit(input);
    setInput('');
    setHistoryIndex(-1);
  };
    ...
    <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-surface via-surface to-transparent">
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
        ...
        <div className="mt-2 text-center text-[11px] text-muted/50">
          ...
          <kbd className="px-1.5 py-0.5 rounded bg-accent/10 text-[10px]">Ctrl+Enter</kbd> {i18n.t('input.send')}
        </div>
      </form>
    </div>
  );
