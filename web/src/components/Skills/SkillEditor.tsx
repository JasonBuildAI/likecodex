...
export function SkillEditor({ skill, onClose }: { skill: Skill | null; onClose: () => void }) {
  ...
  const yamlContent = useMemo(() => {
    ...
    if (!isEdit) { lines.push(`version: ${version || '0.1.0'}`); if (author) lines.push(`author: ${author}`); }
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`flex flex-col min-w-0 ${showPreview ? 'w-1/2' : 'flex-1'}`}>
          {editMode === 'form' ? (
            ...
            <div className="flex-1 min-h-0">
              <MonacoEditor
                value={editMode === 'yaml' ? yamlContent : jsonContent}
                onChange={(v) => {
                  // Parse back from YAML/JSON to form fields (simplified)
                  const text = v || '';
                  if (editMode === 'json') {
                    try {
                  ...
                  // YAML parsing is complex; for simplicity we sync body from yaml content
                }}
                language={editMode === 'yaml' ? 'yaml' : 'json'}
                theme="vs-dark"
                options={{
                  minimap: { enabled: false },
                  fontSize: 13,
        ...
        {/* Right: Allowed tools - only in form mode */}
