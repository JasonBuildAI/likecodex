...
import { fetchSkillDetail, deleteSkill, exportSkill, invokeSkill, importSkill } from '@/lib/api';
...
export function SkillDetailView({ skill, onBack }: { skill: Skill; onBack: () => void }) {
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast({ type: 'success', message: `Imported ${res.imported?.length || 0} skill(s)` });
      // Reload skills
      ...
      <div className="p-3 border-b border-border">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5 shrink-0">
            {/* Import button */}
            <button onClick={handleImportClick} className="px-2 py-1 text-xs rounded bg-surface-hover hover:bg-surface-active transition" title="Import skill from zip">
              ↑ Import
            </button>
            <input
              ref={fileInputRef}
